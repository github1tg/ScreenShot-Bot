...
class Utilities:
    ...
    async def display_settings(c, m, db, cb=False):
        ...
        if as_file:
            as_file_btn.append(
                InlineKeyboardButton("🗂️ Uploading as Document 🗂️.", "set+af")
            )
        else:
    ...
    def gen_ik_buttons():
        ...
        for i in range(1, 21):
            ...
            if i == 21:
                btns.append(i_keyboard)
        btns.append([InlineKeyboardButton("✂️𝑴𝒂𝒏𝒖𝒂𝒍 𝑺𝒄𝒓𝒆𝒆𝒏𝒔𝒉𝒐𝒕𝒔✂️", "mscht")])
        btns.append([InlineKeyboardButton("⚙️𝑻𝒓𝒊𝒎 𝑽𝒊𝒅𝒆𝒐⚙️", "trim")])
        btns.append([InlineKeyboardButton("📝𝑴𝒆𝒅𝒊𝒂 𝑰𝒏𝒇𝒐𝒓𝒎𝒂𝒕𝒊𝒐𝒏📝", "mi")])
        return btns
